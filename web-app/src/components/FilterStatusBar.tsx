    ...
    <div className="flex gap-2 overflow-x-auto pb-1">
      {FILTERS.map((f) => {
        ...
        const count = f.key === 'SEMUA'
          ...
          : f.key === 'AMAN' ? counts.aman : f.key === 'WASPADA' ? counts.waspada : counts.bahaya;
        return (
          <button
            key={f.key}
            ...
            className={cn(
              ...
              isActive ? f.activeClass : 'border-divider text-text-secondary hover:border-primary/40 hover:text-text-primary',
            )}
          >
            {f.label}
