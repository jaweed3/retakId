...
export function ReportsPage() {
  ...
  const { data, counts, totalCount, isLoading, error, refetch } = useLaporan({ status: statusFilter, limit: PAGE_SIZE, page });
  const totalPages = Math.max(1, Math.ceil(totalCount / PAGE_SIZE));
  const filtered = data.filter((r) => !search || r.nama_lokasi.toLowerCase().includes(search.toLowerCase()) || r.pelapor.toLowerCase().includes(search.toLowerCase()));

  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      <div className="mb-6">
        <h2 className="text-lg font-bold text-text-primary mb-1">Daftar Laporan</h2>
        <p className="text-sm text-text-secondary">Semua laporan retakan tanah dari masyarakat.</p>
      </div>
      ...
      {!isLoading && !error && filtered.length === 0 && (
        <EmptyState
          ...
          description={statusFilter === 'SEMUA' ? 'Laporan dari aplikasi mobile akan muncul di sini.' : `Belum ada laporan dengan status "${statusFilter}".`}
        />
      )}
      ...
      {!isLoading && !error && filtered.length > 0 && (
        <>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((r) => <LaporanCard key={r.id} report={r} />)}
          </div>
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-6">
              ...
              <span className="text-xs text-text-secondary tabular-nums">{page + 1} / {totalPages}</span>
